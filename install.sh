...
uptate_pacman_mirrorlist() {
	local country_code=$(echo "$mirrorlist_country" | awk -F'[()]' '{print $2}')
	local mirrorlist_url="https://archlinux.org/mirrorlist/?country=$country_code&protocol=http&protocol=https&ip_version=4&use_mirror_status=on"
